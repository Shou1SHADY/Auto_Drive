...
def perception_step(Rover):
    ...
    bottom_offset = 6 
    source = np.float32([[14, 140], [302 ,140],[204, 95], [116, 95]])
    ...
    mask2 = mask[35:160, 80:240]
    # Adding a black border to cropped mask to regain original shape of (160, 320)
    mask2 = cv2.copyMakeBorder(
        mask2, 35, 0, 80, 80, cv2.BORDER_CONSTANT, value=(0, 0, 0))
    obstacles = np.absolute((np.float32(navigable)-1) * mask2)
    navigable = navigable*mask2
        # ignore half of the image as bad data
    ...
    if (Rover.pitch < 1 or Rover.pitch > 359) and (Rover.roll < 1 or Rover.roll > 359):
        ...
        nav_pix = Rover.worldmap[:, :, 2] > 0  # blue pixels bool array
        tot = len(nav_pix[0])
        Rover.worldmap[nav_pix, 0] = 0
        # clip to avoid overflow
        Rover.worldmap = np.clip(Rover.worldmap, 0, 255)

        #map = np.zeros_like(Rover.worldmap)
        #map[:, :, 2] = nav_pix
        #navigable = Rover.worldmap[:, :, 2] * (255 / np.mean(Rover.worldmap[nav_pix, 2]))   #int array

        #map = plotmap.clip(0, 255)
        
        
        #if any(x in obstacle_x_world for x in navigable_x_world) :
           #if any(y in obstacle_y_world for y in navigable_y_world):
            #Rover.worldmap[obstacle_y_world, obstacle_x_world, 0] = 255
                #Rover.worldmap[navigable_y_world, navigable_x_world, 0] = 255
                #Rover.worldmap[obstacle_y_world, obstacle_x_world, 2] = 0  # increase fidelity
            #Rover.worldmap[obstacle_y_world, obstacle_x_world, 2] = 255


        Rover.worldmap[obstacle_y_world, obstacle_x_world, 0] = 255

        #Rover.worldmap[navigable_y_world, navigable_x_world, 0] -= 90       #increase fidelity
        ...
        Rover.worldmap[obstacle_y_world, obstacle_x_world, 2] -= 3 # increase fidelity
